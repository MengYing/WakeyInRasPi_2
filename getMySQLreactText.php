...
// 設定文件utf-8編碼
 ...
 $sql = "SELECT action.userID,action.actionID,action.date,event.eventType,action.eventID,action.actionType,action.text,event.startTime FROM  action,event WHERE action.eventID = event.eventID AND action.date = $date AND action.actionType = 1 AND action.userID = $userID AND action.deleteOrNot = 1 AND action.text != ''  ";
...
// echo "<table border='1'>";
 ...
 echo $row['actionID'].' '.'userID '.$row['userID'].' '.'eventType'.' '.$row['eventType'].' '.'eventID'.' '.$row['eventID'].' '.'actionType'.' '.$row['actionType'].' '.'date'.' '.$row['date'].' '.$row['text'].' '.'startTime'.' '.$row['startTime']."\n" ;
 }
// echo gettype($row);
// echo gettype($result);
 //echo gettype($result);
