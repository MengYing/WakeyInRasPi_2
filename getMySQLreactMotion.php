...
$userID = 0 ;
$date = "20160502";
//$dateTmp=date_create($date2);
//date_add($dateTmp,date_interval_create_from_date_string("1 days"));
//$date =  date_format($dateTmp,"Ymd");
//$date = "20160423" ;
//$date = "20160410";
//$deleteOrNot;
...
// 設定文件utf-8編碼
 ...
 $sql = "SELECT action.userID,action.date,action.eventID,action.actionID,event.eventType,action.actionType,action.motion,event.startTime FROM  action,event WHERE action.eventID = event.eventID AND action.date = $date AND action.actionType = 4 AND action.userID = $userID AND action.deleteOrNot = 1 AND action.motion !='' ";
...
// echo "<table border='1'>";
 ...
 echo $row['actionID'].' '.'userID '.$row['userID'].' '.'eventType'.' '.$row['eventType'].' '.'eventID'.' '.$row['eventID'].' '.'actionType'.' '.$row['actionType'].' '.'date'.' '.$row['date'].' '.$row['motion'].' '.'startTime'.' '.$row['startTime']."\n" ;
 }
// echo gettype($row);
// echo gettype($result);
 //echo gettype($result);
