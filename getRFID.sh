...
then
echo $tag7
omxplayer -o local /home/pi/test/version1/Wakey_shell/Music/music27.mp3
fi

if [ $RFID = $tag37 ]
then
echo $tag7
omxplayer -o local /home/pi/test/version1/Wakey_shell/Music/music28.mp3
fi

if [ $RFID = $tag38 ]
then
echo $tag7
omxplayer -o local /home/pi/test/version1/Wakey_shell/Music/music30.mp3
fi
...
if [ $RFID = $tag1 ]
then
 for item in $(ls -l  eventType02/event/*.sh)
  do 
    sh ${item}
    rm ${item}
  done
...
if [ $RFID = $tag2 ]
then
 for item in $(ls -l  eventType03/event/*.sh)
  do 
    sh ${item}
    rm ${item}
  done
...
if [ $RFID = $tag3 ]
then
 for item in $(ls -l  eventType04/event/*.sh)
  do 
    sh ${item}
    rm ${item}
  done
...
if [ $RFID = $tag4 ]
then
 for item in $(ls -l  eventType05/event/*.sh)
  do 
    sh ${item}
    rm ${item}
  done
...
if [ $RFID = $tag5 ]
then
 for item in $(ls -l  eventType06/event/*.sh)
  do 
    sh ${item}
    rm ${item}
  done
