...
// 設定文件utf-8編碼
 ...
 $sql = "SELECT action.userID,action.actionID,action.date,event.eventType,action.eventID,action.actionType,action.recording,event.startTime FROM  action,event WHERE action.eventID = event.eventID AND action.date = $date AND action.actionType = 2 AND action.userID = $userID AND action.deleteOrNot = 1 AND action.recording != ''  ";
...
// echo "<table border='1'>";
 ...
 echo $row['actionID'].' '.'userID '.$row['userID'].' '.'eventType'.' '.$row['eventType'].' '.'eventID'.' '.$row['eventID'].' '.'actionType'.' '.$row['actionType'].' '.'date'.' '.$row['date'].' '.$row['recording'].' '.'startTime'.' '.$row['startTime']."\n" ;
 }
// echo gettype($row);
// echo gettype($result);
 //echo gettype($result);
